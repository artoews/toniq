...
def nonrigid(fixed_image, moving_image, fixed_mask, moving_mask, verbose=True):
    ...
    default_bspline_parameter_map['FinalGridSpacingInPhysicalUnits'] = ['20']
    # default_bspline_parameter_map['Metric'] = ['AdvancedMeanSquares']
    # default_bspline_parameter_map['Metric'] = ['AdvancedNormalizedCorrelation']
    default_bspline_parameter_map['Metric'] = ['AdvancedMattesMutualInformation']
    # default_bspline_parameter_map['Metric'] = ['NormalizedMutualInformation']
...
def get_deformation_field(moving_image, transform):
    ...
    return np.asarray(field).astype(np.float)
