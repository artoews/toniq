...
if __name__ == '__main__':
    ...
    gd1 = -np.load(path.join(args.out1, 'gd-map.npy'))[..., 0]
    gd2 = -np.load(path.join(args.out2, 'gd-map.npy'))[..., 0]
    gd1_mask = np.load(path.join(args.out1, 'gd-map-mask.npy'))
    gd2_mask = np.load(path.join(args.out2, 'gd-map-mask.npy'))
