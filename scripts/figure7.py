...
if __name__ == '__main__':
    ...
    # metal = np.load(path.join(args.out, 'gd-metal-rigid-registered-masked.npy'))
    # metal_mask = (metal != 0)
    result = np.load(path.join(args.out, 'gd-metal-registered-masked.npy'))
    gd_map = np.load(path.join(args.out, 'gd-map.npy'))
    ia_map = np.load(path.join(args.out, 'ia-map.npy'))
    ia_map_registered = np.load(path.join(args.out, 'ia-map-registered.npy'))
    ...
    im, _, ax1, ax2 = imshow2(axes[0], -gd_map[..., 0], slc1, slc2, vmin=-args.limit, vmax=args.limit, mask=~result_mask, cmap=CMAP['distortion'])
